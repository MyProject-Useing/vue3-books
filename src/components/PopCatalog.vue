...
export default {
  ...
  computed: {
    index() {
      return this.$store.state.caches.readingBook.index;
    },
    catalog() {
      return this.$store.state.caches.readingBook.catalog || [];
    },
    cataList() {
      if (this.asc) {
        return this.catalog;
  ...
  methods: {
    isSelected(index) {
      if (this.asc) {
        return index == this.$store.state.caches.readingBook.index;
      } else {
        return (
          this.catalog.length - 1 - index ==
          this.$store.state.caches.readingBook.index
        );
      }
    },
    gotoChapter(note) {
