...
export default {
  ...
  data() {
    return {
      index: this.$store.state.caches.readingBook.index,
      bookSource: [],
      loading: false,
      loadingMore: false,
      lastIndex: 0,
  ...
  methods: {
    isSelected(searchBook) {
      return searchBook.bookUrl == this.$store.state.caches.readingBook.bookUrl;
    },
    getBookSource(refresh) {
      this.axios
        .get(this.api + `/getBookSource`, {
          params: {
            url: this.$store.state.caches.readingBook.bookUrl,
            refresh: refresh ? 1 : 0,
          },
        })
        .then(
    ...
    changeBookSource(searchBook) {
      this.axios
        .post(this.api + `/saveBookSource`, {
          bookUrl: this.$store.state.caches.readingBook.bookUrl,
        ...
        .then(
          (res) => {
            if (res.data.isSuccess) {
              ...
              // 重新加载书架
              this.axios.get(this.api + `/getBookshelf`, {}).then(
                (res) => {
                  if (res.data.isSuccess) {
    ...
    jumpToActive() {
      this.$nextTick(() => {
        ...
        this.bookSource.some((v, i) => {
          if (v.bookUrl == this.$store.state.caches.readingBook.bookUrl) {
            index = i;
            return true;
          }
        });
